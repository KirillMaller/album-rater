...
function parseAlbum(album, sourceUrl, albumId, trackId) {
  ...
  return {
    ...
    year: album.year,
    genre: album.genre,
    coverUrl: coverUrl(album.coverUri),
    tracks: finalTracks,
    sourceUrl,
  };
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    const { albumId, trackId } = extractIds(sourceUrl);
    const album = await fetchAlbum(albumId);
    const result = parseAlbum(album, sourceUrl, albumId, trackId);
