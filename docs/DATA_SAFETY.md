...
## Передача каталога Рифмабесу — отдельный чек-лист
...
3. **Завести Рифмабесу собственный аккаунт** в Supabase Auth (через регистрацию на `/admin` под его email) — **не отдавать ему пароль Кирилла**. Добавить его `user_id` в `admin_users` через SQL Editor:
   ```sql
